...
static int parse_rpmc_parameter_table(struct flashchip * const chip, const uint8_t * const buf, const uint16_t len)
{
	...
	chip->rpmc_ctx.update_rate = 5 * (2 << ((first_dword & (0xf << 24)) >> 24));
...
int probe_spi_sfdp(struct flashctx *flash)
{
	...
	for (i = 0; i <= nph; i++) {
		...
		switch (hdrs[i].id){
			case 0: // Mandatory JEDEC SFDP parameter table
			...
			case 0x03: // RPMC parameter table as specified in JESD260
				if (hdrs[i].v_major != 1) {
					msg_cdbg("The chip contains an unknown version of the RPMC parameter parameters table, skipping it.\n");
				} else if (parse_rpmc_parameter_table(flash->chip, tbuf, len) == 0) {
					ret = 1;
				}
				break;
			default:
				msg_cdbg("Support for SFDP Page with ID 0x%02x not implemented, skipping it.\n", hdrs[i].id);
				break;
		}
